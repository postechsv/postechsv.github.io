...
Predicate abstraction is applied to group multiple states that share the same boolean features. This abstraction yields a Markov Decision Process (MDP) to which reinforcement learning is applied. The resulting MDP contains fewer states than the original system, reducing search complexity and allowing learned heuristics to generalize from smaller models to larger, more complex systems.
