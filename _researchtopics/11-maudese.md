...
Maude-SE provides various analysis commands, including *check*, *show model*, *smt-search*, and *show smt-path*. The *check* command determines the satisfiability of Boolean formulas. The *show model* command returns the satisfying assignment, if any, for the last check command. The *smt-search* command performs symbolic reachability analysis, potentially with folding. Finally, the *show smt-path* command displays a path for the last search result. Given a module M, a Boolean formula ψ, and an (optional) SMT theory Th, the check command determines the satisfiability of ψ under the theory Th:
